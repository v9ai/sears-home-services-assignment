...
- [ ] `app/tools/scheduling_tools.py` — add a `_sessions_table` read-mirror (same
      ...
      docstring note 2.
- [ ] `app/voice/recording.py` — `ensure_voice_session_row(session)`: get-or-create the
      minimal phone `sessions` row; best-effort with `voice.session_row.ensure_failed`
      event on error.
- [ ] `app/voice/bot.py` `_on_connected` — fire `ensure_voice_session_row` as a held
      background task (off the greeting critical path).
- [ ] `app/voice/tools.py` — correct the `book_appointment` bridge comment (the
      contextvar read + start-row is the real mechanism).
- [ ] `evals/live_driver.py` — add `appointments_booking_probe()` returning a
      `BookingProbe` that SELECTs an `appointments` row by `session_id`.

## 5. Gates
- [ ] New `tests/scheduling/test_booking_session_attribution.py` green (attributed /
      missing-row fallback / no-session fallback / probe).
- [ ] Voice-side test for `ensure_voice_session_row` idempotence green.
- [ ] `make lint` + `make test` clean; `make transcript` clean.

## 6. Deploy                                           [if deploy in scope]
- [ ] No deploy. Record as roadmap Phase 11; tick `[x]` when the DoD holds. Manual DB
