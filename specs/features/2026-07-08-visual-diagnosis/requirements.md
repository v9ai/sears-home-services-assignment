...
- `send_image_upload_link(email)` tool: creates a tokenized upload row and emails
  `{APP_BASE_URL}/upload/{token}`.
- Mobile-friendly upload page `GET /upload/{token}` + `POST /api/upload/{token}`
  (multipart; 10 MB cap; jpeg/png/webp allowlist; expiry + single-use enforced).
