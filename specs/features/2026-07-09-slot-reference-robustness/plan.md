...
- [x] `app/tools/scheduling_tools.py` — per-session ref cache
...
- [x] `app/agent/prompts.py` `SCHEDULING_CONTRACT` — the model may pass the exact
      `slot_id` or the slot's `ref`.

## 5. Gates
- [x] New tests in `tests/scheduling/test_slot_references.py` green.
- [x] `tests/test_prompts_scheduling.py` updated + green.
- [x] `make lint` + `make test` clean; `make transcript` clean.
- [x] Live adaptive booking drive completes an ATTRIBUTED booking (evidence in
      validation.md).

## 6. Deploy                                           [if deploy in scope]
- [x] No deploy. Recorded with Phase 11 in `specs/constitution/roadmap.md`.
