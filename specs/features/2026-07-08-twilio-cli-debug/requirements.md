...
status), the tunnel (ngrok), the app (structured `twilio.*` events, telephony plan 5b,
correlated by `call_sid`/`session_id`), and the DB/recordings. This toolkit joins them
from one CLI.
...
| `call <CallSid>` | Call detail (`api:core:calls:fetch`) + correlated app-side view: grep `twilio.*` events by `call_sid`, resolve `call_sid → session_id`, point at the `sessions` row and `RECORDINGS_DIR/{session_id}/` | no |
...
5. **`call_sid` is the correlation key** across Twilio API ↔ app logs ↔ `sessions` ↔
   recordings — as established by plan 5b's `TwilioTraceContext`.
