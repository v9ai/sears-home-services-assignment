...
- **Web** (`/ws/call`): a headless text WebSocket test/eval surface — no browser client
  ...
  not a shipped UI.
- **Phone** (`/ws/twilio`, the primary review surface): Twilio Media Streams carries base64 μ-law 8 kHz
...
into the system prompt every turn. This makes "never re-ask" — the assignment's
conversation-memory requirement — a structural property of the context window, not a
...
**Booking integrity** (a hard rule of this build): `book_appointment` claims a slot with
...
1. **Text-harness-first, not phone-first.** Tier 1 ships on text + TTS playback before
   ...
   on audio-codec and VAD complexity. The phone channel is additive: same agent and
